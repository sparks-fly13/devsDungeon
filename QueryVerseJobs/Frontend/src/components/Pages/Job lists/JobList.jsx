...
const ImageCard = () => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        ...
        console.log("The url we are using to fetch image = ", url)
        setImage(url);
      } catch (error) {
  ...
  return (
    <div className="relative m-0 overflow-hidden text-gray-700 bg-white dark:text-gray-300 dark:bg-[#212734] rounded-none sm:rounded-md lg:rounded-xl bg-clip-border hidden sm:block w-full md:w-1/3 lg:w-1/3">
      {image ? (
        <img
          src={`https://api.unsplash.com/photos/random?client_id=${accessKey}&query=company`}
          alt="card-image"
          className="object-cover w-auto h-[20vw] md:h-[12vw] lg:h-[15vw] xl:h-[17vw] sm:rounded-none lg:rounded-none xl:rounded-none"
        />
      ) : (
        <p className="text-center p-4">Loading image...</p>
      )}
    </div>
  );
};
